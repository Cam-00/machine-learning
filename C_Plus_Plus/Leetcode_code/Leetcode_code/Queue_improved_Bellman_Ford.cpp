/*
	基于队列优化的Bellman_Ford算法
	核心思想：对上一次松弛的时候更新过的节点作为出发节点所连接的边 进行松弛就够了
	时间复杂度： O(N * E) N 为节点数量，E为边的数量。
			 最坏的情况下是 O(N * E)，但 一般情况下 时间复杂度为 O(K * N)

	有环且只有正权回路的情况下，即使元素重复加入队列，最后，也会因为 所有边都松弛后，
	节点数值（minDist数组）不在发生变化了 而终止。
